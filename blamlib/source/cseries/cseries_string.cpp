...
{
	assert(string);
	assert(max_length >= 0 && max_length <= k_maximum_string_size);

	unsigned int remaining = max_length;

	for (unsigned int remaining = max_length;
		(remaining > 0) && (string[remaining - 1] != '\0');
		remaining--)
	{
		string[remaining - 1] = ascii_tolower(string[remaining - 1]);
	}
...
char *csstrnupr(char *string, unsigned int max_length)
{
	...
	for (unsigned int remaining = max_length;
		...
		remaining--)
	{
		string[remaining - 1] = ascii_toupper(string[remaining - 1]);
	}
