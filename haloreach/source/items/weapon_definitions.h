...
{
	real_fraction charge_fraction; // charging fraction at which the weapon should additionally fire a shot
};
...
struct s_weapon_trigger_charging_parameters
{
	...
	c_flags<e_weapon_trigger_charging_flags, ushort> flags;
	...
	c_tag_block<s_weapon_trigger_charging_fire_fraction> fire_fractions;
};
...
{
	_weapon_barrel_tracks_fired_projectile_bit,
	...
	_weapon_barrel_use_error_when_unzoomed_bit,
	...
	_weapon_barrel_cant_fire_when_others_firing_bit,
	_weapon_barrel_cant_fire_when_others_recovering_bit,
	_weapon_barrel_dont_clear_fire_bit_after_recovering_bit,
...
struct s_weapon_barrel_firing_parameters
{
	...
	real_fraction blurred_rate_of_fire; // a percentage between 0 and 1 which controls how soon in its firing animation the weapon blurs
	...
	real_fraction soft_recovery_fraction; // how much of the recovery allows shots to be queued
	real melee_fire_recovery_time; // how long after a set of shots it takes before the weapon can melee
	real_fraction melee_soft_recovery_fraction; // how much of the melee recovery allows melee to be queued
};
...
struct s_weapon_barrel_projectile_distribution
{
	...
	angle distribution_angle; // used by _weapon_barrel_projectile_distribution_horizontal_fan
};
static_assert(sizeof(s_weapon_barrel_projectile_distribution) == 0x8);

struct s_weapon_barrel_projectile_error_parameters
{
	angle minimum_error; // projectile direction is randomly selected between this and max_error_angle below
	angle_bounds error_angle; // current barrel_error is linearly interpolated between these to generate max_error_angle
};
static_assert(sizeof(s_weapon_barrel_projectile_error_parameters) == 0xC);

struct s_weapon_barrel_wield_accuracy_penalties
{
	c_tag_block<s_tag_data> firing_penalty_function; // percentage accuracy lost when the barrel has fired
	c_tag_block<s_tag_data> firing_crouched_penalty_function; // percentage accuracy lost when the barrel has fired from a crouched position
	c_tag_block<s_tag_data> moving_penalty_function; // percentage accuracy lost when moving
	c_tag_block<s_tag_data> turning_penalty_function; // percentage accuracy lost when turning the camera
...
struct s_weapon_barrel_definition
{
	c_flags<e_weapon_barrel_definition_flags, ulong> flags;
	...
	c_tag_block<s_weapon_barrel_first_person_offset> first_person_offset;
	...
	c_tag_block<s_weapon_barrel_firing_effect> firing_effects; // firing effects determine what happens when this trigger is fired
};
...
{
	c_flags<e_weapon_screen_effect_flags, uchar> flags;
	char : 8;
	short : 16;
...
struct s_weapon_definition : s_item_definition
{
	...
	c_tag_block<s_weapon_trigger_definition> new_triggers;
	c_tag_block<s_weapon_barrel_definition> barrels;
	...
	c_tag_block<s_weapon_screen_effect> screen_effects;
};
