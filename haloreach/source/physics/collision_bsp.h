...
{
	c_flags<e_collision_bsp_leaf_flags, uchar> flags;
	char : 8;
	ushort bsp2d_reference_count;
	ulong first_bsp2d_reference;
};
...
{
	real_point3d point;
	c_tag_block_index<s_collision_bsp_edge, ushort> first_edge;
	short sink;
};
...
struct s_collision_bsp_edge
{
	c_tag_block_index<s_collision_bsp_vertex, ushort> start_vertex;
	c_tag_block_index<s_collision_bsp_vertex, ushort> end_vertex;
	c_tag_block_index<s_collision_bsp_edge, ushort> forward_edge;
	c_tag_block_index<s_collision_bsp_edge, ushort> reverse_edge;
	c_tag_block_index<s_collision_bsp_surface, ushort> left_surface;
	c_tag_block_index<s_collision_bsp_surface, ushort> right_surface;
};
...
struct s_collision_bsp_surface
{
	c_tag_block_index<real_plane3d, ushort> plane;
	c_tag_block_index<s_collision_bsp_edge, ushort> first_edge;
	short material_index; // TODO: block index
	c_enum<e_collision_bsp_surface_flags, uchar> flags;
	uchar best_plane_calculation_vertex_index;
};
static_assert(sizeof(s_collision_bsp_surface) == 0x8);

struct s_collision_bsp
{
	c_tag_block<s_bsp3d_node> bsp3d_nodes;
	c_tag_block<s_bsp3d_kd_supernode> bsp3d_supernodes;
	c_tag_block<real_plane3d> planes;
	c_tag_block<s_collision_bsp_leaf> leaves;
	c_tag_block<s_bsp2d_reference> bsp2d_references;
	c_tag_block<s_bsp2d_node> bsp2d_nodes;
	c_tag_block<s_collision_bsp_surface> surfaces;
	c_tag_block<s_collision_bsp_edge> edges;
	c_tag_block<s_collision_bsp_vertex> vertices;
};
